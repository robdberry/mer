...
pub fn terminal_theme(palette: &Palette) -> Option<Value> {
    ...
    // One `json!` per diagram family: a single object this large exceeds the macro's recursion
    // limit.
    let groups = [
        json!({
            "darkMode": dark,
            "fontFamily": crate::fonts::FAMILY,
            "background": paper,
            "textColor": ink,
            "titleColor": ink,
            "lineColor": line,
